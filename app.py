...
if 'logged_in' not in st.session_state:
    ...
    access_map = {
        ...
        'مركز صحي حديثة': 'HAD-HEALTH-2026',
        'مركز صحي بروانه': 'BAR-HEALTH-2026',
        'مركز صحي حقلانية': 'HAK-HEALTH-2026',
        'مركز صحي بني داهر': 'DAH-HEALTH-2026',
        'مركز صحي الوس': 'WAS-HEALTH-2026',
        ...
        if data.data:
            for p in data.data:
                st.markdown(f"""<div class="patient-card">
                    ...
                    <p style='margin:5px 0;'>🔬 الإصابة: <b>{p['infection_type']}</b> | 🏢 المركز: {p['entry_center']}</p>
                        ...
                        if st.session_state.is_admin or p['entry_center'] == st.session_state.center:
                            ...
                            with c2:
                                with st.form(f"edit_{p['id']}"):
                                    ...
                                    new_age = st.text_input("تعديل العمر:", value=p.get('age'))
                                    new_phone = st.text_input("تعديل الهاتف:", value=p.get('phone_number'))
                                    if st.form_submit_button("حفظ التعديلات"):
                                        supabase.table("patients").update({"full_name": new_name, "age": new_age, "phone_number": new_phone}).eq("id", p['id']).execute()
                                        st.rerun()
